...
export default {
  ...
  async asyncData({ store, params }) {
    const docRef = doc(db, "admins", params.edit);
    await getDoc(docRef).then((res) => {
      store.dispatch("admin/adminsWorkers/edit/getAllDataFromApi", res);
    });
    return {};
  },
  data() {
    return {
      module: "admin/adminsWorkers/edit",
    };
  },
  methods: {
    editData() {
      this.$refs.form.validate().then((res) => {
        if (res) {
          this.$store
            .dispatch(
              "admin/adminsWorkers/edit/editInDB",
              this.$route.params.edit
            )
            .then(() => {
              this.$router.push("/dashboard/admins-Worker");
            });
        }
      });
    },
